...
- Use the shared helpers such as `log`, `error`, `require_command` etc unless a script has a clear reason to do otherwise.
...
- For local test composition in this repository, declare features in dependency order to avoid using `overrideFeatureInstallOrder`.

## Feature Testing

- Use `./scripts/test.sh` to test feature changes and the `_global` integration test. This script is a token-efficient wrapper for running common `devcontainer features test ...` commands. It captures detailed output in a temporary development/testing log file and prints the test report.
...
- Write temporary files used in development/testing to `/tmp/<repo-name>/` and remove them when no longer needed.

## Docker Resources

- After completing changes, remove outdated/superceded Docker resources (images, containers, volumes, networks, etc) where no longer reusable.
